...
def function3():
    ...
    a = a.reshape(1,1000) #wrtie your code here
    x = a[::,34:1001:35]
    return x
"""
    Expected Output:
...
def function8():
    ...
    x = np.arange(0,10,dtype=np.int32) #write your code here
    x[x] = 6
    x=x.reshape(2,5)
    return x
...
def function12():
    ...
    arr = np.array([1,2,3])
    arr1 = np.copy(arr)#write your code here 
    x =np.stack((arr,arr,arr),axis=1)
    y =np.stack((arr1,arr1,arr1),axis=0)
    ans=np.stack((x,y),axis=0)
    ans=ans.ravel()
    return ans
...
def function13():
    ...
    x=arr[arr>5] #write your code here 
    ans=x[x<10]
    return ans
...
def function14():
    ...
    arr = arr.reshape(8,3) #write reshape code
...
def function18():
    ...
    x = (students <= 100).sum() # Write you code Here
    return x
